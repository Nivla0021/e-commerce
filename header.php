    ...
    <div class="header">
        <div class="sec1">
            ...
            if ($_SESSION['role'] == 'user') {
                echo <<< HTML
                        <a href="userDashboard.php"><img src="pics/bastalogo.png" alt="logo" class="nav_logo"></a>
                    HTML;
            } else {
                echo <<< HTML
        ...
        <div class="sec2">
            <a href="userDashboard.php" class="pr">
                <ul class="nav_links">
                    <li><a href="userDashboard.php" class="pr">Products</a></li>
                </ul>
            </a>
        ...
        <div class="nav_img">
            <div class="mess_section">
                <a href="userMessage.php"><img src="pics/mes.png" alt="message" class="userMessage"></a>
                <?php
                include 'conn2.php';
                ...
                FROM Messages
                WHERE receiverId = '$currentUserId' AND is_read = FALSE";
                $notif = 0;
                $result = $conn->query($sql);

                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $notif = $row['unread_count'];
                    }
                    if ($notif < 1) {
                        echo <<<HTML
                        <p class="notif" style="display: none"></p>
                        HTML;
                    } else if ($notif > 9) {
                        echo <<<HTML
                        <p class="notif" style="padding: 3px 7px 3px 3px;">9+</p>
                        HTML;
                    } else {
                        echo <<<HTML
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            if ($rowcount > 9) {
                                ...
                                <p id="item_qty" style="padding: 2px 20px 2px 2px;">9+</p>
                             HTML;
                            } else if ($rowcount < 1) {
                            } else {
                                echo <<<HTML
